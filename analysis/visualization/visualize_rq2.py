...
def _plot_game_specific_adaptation(magic_df, plots_dir):
    ...
    for game in magic_df['game'].unique():
        ...
        if len(metrics) == 2:
            ...
            sns.scatterplot(data=pivot_df, x=metric1, y=metric2, hue='model', style='structural_variation', s=200, palette='tab10')
            ...
            plt.title(f"{game.replace('_', ' ').title()}: Strategic Profile", fontsize=16)
            ...
            plt.savefig(plots_dir / f"P2.1_{game}_strategic_map_scatter.png")
            plt.close()

        # --- If 3+ metrics, use a Radar Chart ---
        elif len(metrics) >= 3:
            for variation in game_df['structural_variation'].unique():
                ...
                for model in sorted(pivot_df.index):
                    values = pivot_df.loc[model].tolist() + [pivot_df.loc[model].tolist()[0]]
                    ax.plot(angles, values, label=model, linewidth=2)
                    ax.fill(angles, values, alpha=0.25)
                
                ax.set_yticklabels([])
                ax.set_xticks(angles[:-1])
                ax.set_xticklabels([l.replace('_',' ').title() for l in labels], size=12)
                plt.title(f"{game.replace('_',' ').title()} Profile ({variation})", size=16, y=1.1)
                plt.legend(loc='upper right', bbox_to_anchor=(1.4, 1.1))
                plt.tight_layout()
                plt.savefig(plots_dir / f"P2.1_{game}_radar_profile_{variation}.png")
                plt.close()
...
def _plot_collusion_stability(results_dir, plots_dir):
    ...
    sns.lineplot(data=collusion_prop, x='round', y='proportion', hue='model', style='condition_type', lw=2.5, palette='tab10', markers=True, dashes=True)
...
def _plot_nuanced_reporting_strategy(results_dir, plots_dir):
    ...
    plt.figure(figsize=(14, 8))
    sns.lineplot(data=rates_df, x='round', y='deception_event', hue='model', style='condition_type', palette='viridis', lw=2.5, markers=True)
    plt.title("Deception Rate Over Time (Athey & Bagwell)", fontsize=16)
    plt.xlabel("Game Round", fontsize=12)
    plt.ylabel("Proportion of 'Low' Reports when Cost is High", fontsize=12)
    plt.legend(title='Model & Condition', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.tight_layout(rect=[0, 0, 0.85, 1])
    plt.savefig(plots_dir / "P3.3_athey_bagwell_deception_strategy.png")
    plt.close()
...
def _plot_reporting_strategy_matrix(results_dir, plots_dir):
    ...
    for model in strategy_df['model'].unique():
        for condition in strategy_df['condition'].unique():
            ...
            pivot_df = plot_data.pivot_table(index='challenger_true_cost', columns='challenger_report', values='probability').fillna(0).reindex(['high', 'low']).reindex(['high', 'low'], axis=1)
            
            plt.figure(figsize=(8, 6))
            sns.heatmap(pivot_df, annot=True, fmt=".2f", cmap="YlGnBu", linewidths=.5, cbar=False, vmin=0, vmax=1)
            plt.title(f"Reporting Strategy: {model}\n({condition})", fontsize=16)
