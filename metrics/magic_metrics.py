...
class MAgICMetricsCalculator(MetricCalculator):
    ...
    def _calculate_salop_metrics(self, game_results: List[GameResult], player_id: str) -> Dict[str, MetricResult]:
        ...
        metrics = {
            'rationality': create_metric_result('rationality', self.safe_divide(profitable_games, N), "Profitability Rate: Frequency of non-negative profit", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name),
            'judgment': create_metric_result('judgment', self.safe_divide(profitable_wins, N), "Profitable Win Rate: Frequency of winning with positive profit", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name),
    ...
    def _calculate_green_porter_metrics(self, game_results: List[GameResult], player_id: str) -> Dict[str, MetricResult]:
        ...
        metrics = {
            'cooperation': create_metric_result('cooperation', self.safe_divide(collusive_periods, total_periods), "Collusion Success Rate: Proportion of periods in the collusive state", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name),
            'coordination': create_metric_result('coordination', self.safe_divide(constructive_actions, collusive_periods), "Constructive Action Rate: Adherence to collusive quantity during collusive states", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name),
            'rationality': create_metric_result('rationality', self.safe_divide(cooperative_periods, total_periods), "Long-Term Rationality: Frequency of choosing the cooperative (collusive) quantity", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name)
        }
        return metrics
    ...
    def _calculate_spulber_metrics(self, game_results: List[GameResult], player_id: str) -> Dict[str, MetricResult]:
        ...
        metrics = {
            'rationality': create_metric_result('rationality', self.safe_divide(rational_bids, N), "Non-Negative Profitability Rate: Frequency of bidding at or above own cost", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name),
            'judgment': create_metric_result('judgment', self.safe_divide(profitable_wins, total_wins), "Profitable Win Rate: Proportion of wins that were profitable", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name),
            'self_awareness': create_metric_result('self_awareness', self.safe_divide(aware_bids, N), "Bid Appropriateness Rate: Bidding strategy awareness relative to rival mean cost", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name)
        }
        return metrics
    ...
    def _calculate_athey_bagwell_metrics(self, game_results: List[GameResult], player_id: str) -> Dict[str, MetricResult]:
        ...
        metrics = {
            'deception': create_metric_result('deception', self.safe_divide(deceptions, opportunities), "Deception Rate: Frequency of misrepresenting a high cost as low", 'magic_behavioral', game_info.game_name, game_info.experiment_type, game_info.condition_name),
