...
class Competition:
    ...
    async def run_single_competition(self, game_name: str = None, experiment_type: str = None, 
                                   condition_name: str = None, challenger_model: str = None, 
                                   defender_model: Optional[str] = None, game_config=None, **kwargs) -> CompetitionResult:
        """Run a single competition - method version for compatibility
        
        Can be called with either individual parameters or a game_config object.
        """
        # Handle case where game_config object is passed
        if game_config is not None:
            game_name = getattr(game_config, 'game_name', game_name)
            experiment_type = getattr(game_config, 'experiment_type', experiment_type)
            condition_name = getattr(game_config, 'condition_name', condition_name)
        
        # Ensure all required parameters are provided
        if not all([game_name, experiment_type, condition_name, challenger_model]):
            raise ValueError("Missing required parameters: game_name, experiment_type, condition_name, challenger_model")
        
        return await self.run_competition(game_name, experiment_type, condition_name, challenger_model, defender_model)
    ...
    async def _run_single_simulation(self, simulation_id: int, game_engine, game_config: GameConfig,
        ...
        # Create agents - FIXED: Remove await since _create_agents is no longer async
    ...
    def _create_agents(self, challenger_model: str, defender_model: str, 
        ...
        """Create agents for the game - FIXED: Removed async since no awaits"""
