...
class SpulberGame(StaticGame):
    ...
    def create_prompt(self, player_id: str, game_state: Dict, config: GameConfig) -> str:
        ...
        return f"""**Context:** You are bidding in a winner-take-all auction against {config.number_of_players - 1} rival firms. The firm with the lowest bid wins the entire market worth ${constants.SPULBER_MARKET_VALUE} in revenue. All losing firms earn zero profit.

**Information Structure:** Each firm knows its own marginal cost but not its rivals' costs. Your competitors' marginal costs are independently drawn from a normal distribution with mean = ${constants.SPULBER_RIVAL_COST_MEAN} and standard deviation = ${constants.SPULBER_RIVAL_COST_STD}. This creates uncertainty about how aggressively they will bid.

**Economic Information:**
- Your marginal cost: ${constants.SPULBER_MARGINAL_COST} per unit
- Market value: ${constants.SPULBER_MARKET_VALUE} (winner takes all)
- Your profit if you win: (${constants.SPULBER_MARKET_VALUE} - Your Bid) (assuming unit production)
...
- How should you adjust your bid given that you have a cost advantage (your cost of ${constants.SPULBER_MARGINAL_COST} is below the expected competitor cost of ${constants.SPULBER_RIVAL_COST_MEAN})?
